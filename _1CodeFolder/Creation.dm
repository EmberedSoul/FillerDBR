...
/mob/Players/proc/addMissingSkills()
	...
	/obj/Skills/Buffs/Styles/Style_Selector, /obj/Skills/Transformation, /obj/Skills/Fly)
	for(var/S in missingSkills)
		if(!locate(S, usr.contents))
			usr.AddSkill(new S)
	usr.BuildGiven = 1
	var/list/buildVerbs = typesof(/mob/Mapper/verb) - list(/mob/Mapper/verb/XYZ_Teleport, /mob/Mapper/verb/Make_Warper)
	usr.verbs |= buildVerbs
